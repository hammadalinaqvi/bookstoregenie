...
$query = mysql_query('SELECT * FROM book_shipment WHERE transaction_status = "in_progress" && status = "price" || status = "quantity"');
	...
	while ( $result = mysql_fetch_array($query) )
	{
		...
		$book_quantity = $result['quantity_used'] + $result['quantity_new'];
...
$query_update = mysql_query("UPDATE book_shipment SET to_name='$shipToName', to_addressLine1='$shipToAdd1', to_addressline2='$shipToAdd2', to_city='$shipToCity', to_state='$shipToState', to_country='$shipToCountry', to_postalCode='$shipToPostCode', destination_code='$DestinationFulfillmentCenterId', fulfilment_network_SKU='$fulfillmentNetworkSKU', label_type='$lableType', shipment_id='$shipment_id', status='shipment_plan', transaction_status='in_progress', updated_at='$updated_at' WHERE SKU='$item_SKU'") or die(mysql_error());;
...
curl_close($ch);
	...
	$USPS_shipment_id = $USPSResponse['DeliveryConfirmationV3.0Response']['DeliveryConfirmationNumber']['VALUE'];
	
	$content_USPS_PDF = $USPSLabel;
	$content_Outer_USPS_Slip = base64_decode($content_USPS_PDF);
	
	echo '<br /> Data parsed and decoded.</br /></br />';
	
// Setting the file names for USPS label images	
	$USPS_label_filename = 'uploads/img_'.time();
	
	$filename_tif = $USPS_label_filename.'.tif';
	$filename_png = $USPS_label_filename.'.png';
	
	//echo 'PNG File Name: '.$filename_png.'<br /><br />';

// Putting the TIF image from USPS PDF to local file
	file_put_contents($filename_tif, $content_Outer_USPS_Slip);

// Setting the widht, height, x and y coordinates for cropping
	$width = 1214;
	$height = 825;
	$x = 137;
	$y = 110;

// Converting image from TIF to PNG (requires imagick extension installed on server)
	$image = new Imagick($filename_tif);
	$image->cropImage($width, $height, $x, $y);
	//$image->resizeImage(595, 404, imagick::FILTER_BOX, 0.9, true);
	$image->writeImage($filename_png);
...
$html .= '<tr><td colspan="2" height="60px">
- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - </td></tr>';
$html .= '<tr><td colspan="2"><img src="'.$filename_png.'"></td></tr>';
...
$message->setTo($to);
$message->setBcc(array('hammad.ali@zigron.com' => 'Dev'));
...
$query_book_order_update = mysql_query("UPDATE book_order SET USPS_shipment_id='$USPS_shipment_id' WHERE id=$order_id");
