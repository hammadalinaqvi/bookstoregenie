	 ...
	 */
		...
		switch($reqType){
			case "buyinfo":
				...
				for ($i = 0; $i < count($isbns); $i++){
					$isbn = $isbns[$i];
					if ($isbn.substr(0, 3) == 290 || $isbn.substr(0, 3) == 280){
						$isbn	  = "978" . $isbn.substr(-3);
						...
						$isbn = $isbn.substr(0, -1) . 10 - ($sum % 10);
					}
			...
			case "buyback":
				for ($i = 0; $i < count($isbns); $i++){
					...
					$return_arr[] = parseBuybackInfo(getData("http://api.campusbooks.com/11/rest/buybackprices?key={$apiKey}&isbn={$isbn}&image_height=" . IMAGE_HEIGHT . "&image_width=" . IMAGE_WIDTH), $isbns[$i]); // get the data, parse it, then send it on it to the display function
				}
				$final_arr['books'] = $return_arr;
				break;
		}
	 ...
	 function getInput(){
		...
		if ($reqType == "buyinfo" || $reqType == "buyback"){
			$isbns 		  = str_replace(",", " ", str_replace(", ", " ", str_replace("-", "", $_REQUEST[$BROWSERNAMES['isbn']])));
