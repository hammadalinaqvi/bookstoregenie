<?php
	...
	function buy($college,$org,$contactPerson,$email,$number,$paypal,$code)
	{
		$response = new xajaxResponse();
		
		$mysqli = new mysqli('localhost', 'jteplitz', 'jtt0511');
		
		$query = "select * from jteplitz_bookstore.discounts where Name = '$code'";
		
		$result = $mysqli->query($query);
		
		if ($result->fetch_object() != null) {
			$status = "<font color='red'>You have not entered a valid or unique Genie Code!!  Love, Farhan <font>";
		...
		else
		{
			...
			$query = "insert into jteplitz_bookstore.discounts (Name, Type,CouponCreator) values ('$code',$highest,'farhan')";	
...
<div id = "page">
	<div id = "header">
		<h1>Just for Farhan - Please add lots of organizations - Love, Farhan</h1>
	</div>
	<div id = "content">
	...
	return xajax_buy(college,org,contactPerson,email,number,paypal,code);">
	...
	var paypal=document.getElementById('payPal').value;
	var code=document.getElementById('code').value;
	return xajax_buy(college,org,contactPerson,email,number,paypal,code);"> </input>
		</form>
	</div>
